...
class PaginationFS2 extends FunSuite {
  ...
  test("firstPage") {
    ...
    assert(firstPage.unsafeRunSync == firstPageExpected)
  }

  def stream: Stream[IO, Post] = ???

  test("pagination") {
    pending
    assert(stream.compile.toList.unsafeRunSync.last == Post(99, "11bb3ff7-da60-4ccf-83f7-0db3bdb2ece8", "Downs", "MAXEMIA"))
  }
